...
func SetDoujinshiProgress(database *sql.DB, doujinshiID int64, rating, lastPage int) error {
	_, err := database.Exec(`
		INSERT INTO doujinshi_progress (doujinshi_id, rating, last_page)
		VALUES (?, ?, ?)
		ON CONFLICT(doujinshi_id) DO UPDATE SET
			rating=excluded.rating, last_page=excluded.last_page
	`, doujinshiID, rating, lastPage)
	return err
}

func GetDoujinshiProgress(database *sql.DB, doujinshiID int64) (DoujinshiProgress, error) {
	var dp DoujinshiProgress
	err := database.QueryRow(`
		SELECT doujinshi_id, rating, last_page
		FROM doujinshi_progress
		WHERE doujinshi_id = ?
	`, doujinshiID).Scan(&dp.DoujinshiID, &dp.Rating, &dp.LastPage)
	return dp, err
}
